...
```shell
podman build -t localhost/fraud-analytics .
```
...
```shell
cd app
podman build -t localhost/fraud-analytics-app .
```
...
```
Both these model files will be available in `/opt/artifacts` dir on the machine wher you have run the jupyter notebook. Store these files in a simple HTTP server and pass it to the PIM partition like above.
```shell
# Install httpd
yum install httpd -y
